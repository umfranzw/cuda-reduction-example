...
/* This kernel improves only the last one by using a smarter "linear indexing" approach.
 ...
 * the other half of the array. This means that collectively, the threads will access
 * consective array indices when they read from the right half, and when they write to the left half.
...
__global__ void reduce(float *input, float *output, unsigned int n)
{
    ...
    // As last time, thread 0 write's this block's partial result to the output buffer.
    if (!local_id)
    {
